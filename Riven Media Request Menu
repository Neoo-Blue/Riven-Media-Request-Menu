...
(function() {
    ...
    console.log('🎬 Riven Enhanced Menu v2.6 loaded!');
    ...
    function detectSite() {
        ...
        if (url.includes('letterboxd.com/film')) return 'letterboxd';
        if (url.includes('trakt.tv/movies/') || url.includes('trakt.tv/shows/')) return 'trakt';
        return null;
    }
    ...
    function tryTraktLookup(mediaInfo, callback) {
        ...
        GM_xmlhttpRequest({
            ...
            headers: {
                ...
                'trakt-api-version': '2',
                'trakt-api-key': '64cf24d0dfa7d0e95b0b7d5c7b8276e1e3264a180c1e7c5354e4ba5ac8190b50'
            },
            onload: function(response) {
                try {
                    const data = JSON.parse(response.responseText);
    ...
    function extractLetterboxd() {
        ...
        for (const selector of titleSelectors) {
            ...
            if (element && element.textContent.trim()) {
                title = element.textContent.trim();
                console.log('Found Letterboxd title with selector:', selector, title);
                break;
            }
        }
        ...
        console.log('Letterboxd extraction:', { title, tmdbId, imdbId });
    ...
    function extractTrakt() {
        ...
        // Multiple selectors for Trakt title
        let title = null;
        const titleSelectors = [
            'h1[itemprop="name"]',
            '.summary h1',
            'div[data-type="movie"] h1',
            'div[data-type="show"] h1',
            'h1',
            '.mobile-title h1'
        ];
        ...
        for (const selector of titleSelectors) {
            const element = document.querySelector(selector);
            if (element && element.textContent.trim()) {
                title = element.textContent.trim().replace(/\(\d{4}\)$/, '').trim(); // Remove year
                console.log('Found Trakt title with selector:', selector, title);
                break;
            }
        }
        
        // Try to find IMDb ID from external links section
        let imdbId = null;
        const imdbSelectors = [
            'a[href*="imdb.com/title/"]',
            '.external a[href*="imdb.com"]',
            'a[data-title="IMDb"]'
        ];
        ...
        for (const selector of imdbSelectors) {
            ...
            if (imdbLink) {
                ...
                if (match) {
                    imdbId = match[0];
                    console.log('Found Trakt IMDb ID:', imdbId);
                    break;
                }
            }
        }
        ...
        const tmdbSelectors = [
            ...
            '.external a[href*="themoviedb"]'
        ];
        
        for (const selector of tmdbSelectors) {
            const tmdbLink = document.querySelector(selector);
            if (tmdbLink) {
                const match = tmdbLink.href.match(/\/(movie|tv)\/(\d+)/);
                if (match) {
                    tmdbId = match[2];
                    console.log('Found Trakt TMDb ID:', tmdbId);
                    break;
                }
            }
        }
        
        console.log('Trakt extraction result:', { title, imdbId, tmdbId, type });
        
        return { imdbId, tmdbId, title, type };
    }

    function addMenuToLargestTitle(mediaInfo) {
        console.log('Finding largest title element...');
        ...
        for (const selector of titleSelectors) {
            ...
            elements.forEach(element => {
                ...
                if (fontSize > largestFontSize && text.length > 0 && text.length < 200) {
        ...
        console.log('Largest title found:', largestElement, 'Font size:', largestFontSize);
    ...
    function createRivenMenu(mediaInfo, targetElement) {
        ...
        document.getElementById('riven-request-btn').onclick = (e) => {
            e.stopPropagation();
            if (!config.baseUrl || !config.apiKey) {
                showSettingsDialog();
                return;
        ...
        document.getElementById('riven-refresh-btn').onclick = (e) => {
            e.stopPropagation();
            checkLibraryStatus(mediaInfo);
        };

        document.getElementById('riven-hide-btn').onclick = (e) => {
            e.stopPropagation();
            stopAutoRefresh();
